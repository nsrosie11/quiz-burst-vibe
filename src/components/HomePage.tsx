      ...
      <div className="grid grid-cols-2 gap-4 mt-8">
        <Card className="p-4 bg-card">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-2xl font-nunito font-bold text-foreground">{userTotalScore.toLocaleString()}</p>
        ...
        <Card className="p-4 bg-card">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-2xl font-nunito font-bold text-foreground">#{globalRank || '-'}</p>
        ...
        <Card className="p-6" style={{ backgroundColor: 'var(--recommendation-bg)' }}>
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0">
              {(() => {
                const recommendedCategory = categories.find(cat => cat.id === dailyRecommendation?.category_id);
                const IconComponent = iconMap[recommendedCategory?.id || 'math'] || Calculator;
                return <IconComponent className="w-10 h-10 text-white" />;
              })()}
            </div>
            <div className="flex-1 space-y-3">
              <div>
                <p className="text-white font-nunito font-semibold text-lg">
                  Kerjain {categories.find(cat => cat.id === dailyRecommendation?.category_id)?.name || 'Matematika'} yuk!
                </p>
                <p className="text-white/80 font-nunito text-sm">
                  {dailyRecommendation?.reason || 'Lagi banyak yang main kategori ini hari ini. Kesempatan bagus buat naik rank!'} 🚀
                </p>
              </div>
              <Button 
                style={{ 
                ...
                onClick={() => handleCategoryClick(dailyRecommendation?.category_id || 'math')}
              >
                Mulai Sekarang
              </Button>
            </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {categories.map((category) => {
            const IconComponent = iconMap[category.id] || Star;
            return (
              <Card 
                key={category.id}
